...
bool DrawModelShader::Init()
{
	...
	if (um3nLocation == -1)
	{
		puts("Get uniform loaction error: drawmodelshader-um3n");
		return false;
	}
	...
	norm = GetUniformLocation("norm");
	if (wireColorLocation == -1)
	{
		puts("Get uniform loaction error: norm");
		return false;
	}
	return true;
}
...
void DrawModelShader::SetNorm(const glm::vec3& norm)
{
	glUniform4fv(faceColorLocation, 1, glm::value_ptr(norm));
}
